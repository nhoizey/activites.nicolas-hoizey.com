...
const stravaType = contentRoot
	.querySelector("header .title")
	.childNodes[2].nodeValue.replace(/\n+/g, "")
	.trim()
	.split("–")[1]
	.trim();
let type = stravaType.toLowerCase();

const timeAndDate = activityInfo
	.querySelector("time")
	.textContent.trim()
	.split(",");
...
const description =
	activityInfo
		.querySelector(".activity-description .content")
		?.textContent.trim() || "";

const distance =
	activityStats
		.querySelector('strong:has(abbr[title="kilomètres"])')
		?.childNodes[0].nodeValue.replace(",", ".")
		.trim() || "0";
let duration =
	activityStats
		.querySelector(
			'li:has([data-glossary-term="definition-moving-time"]) strong',
		)
		?.textContent.trim() || "00:00:00";
...
const elevation =
	activityStats
		.querySelector('strong:has(abbr[title="mètres"])')
