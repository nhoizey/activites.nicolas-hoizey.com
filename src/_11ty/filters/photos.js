...
export const addPhotoDimensions = (content, filePathStem, width = false) => {
	...
	const photoCacheDir = path.join('src/_cache/photos', filePathStem.replace(/\/index$/, ''));
	...
	matches.forEach((match) => {
		...
		const photoData = photosData.find(photo => photo.src === cleanSrc && photo.dimensions !== undefined);

		if (photoData) {
			if (width !== false) {
				newContent = newContent.replace(`<img src="${src}"`, `<img src="${src}" width="${width}" height="${Math.round(photoData.dimensions.height * (width / photoData.dimensions.width))}"`);
			} else {
				newContent = newContent.replace(`<img src="${src}"`, `<img src="${src}" width="${photoData.dimensions.width}" height="${photoData.dimensions.height}"`);
			}
		}
	});
