...
const EXIFR_OPTIONS = {
	...
	exif: ["DateTimeOriginal", "OffsetTime", "ExifImageWidth", "ExifImageHeight"],
	gps: {
...
export default {
	...
	eleventyComputed: {
		month: (data) => {
			if (
				!data.page.filePathStem.match(/^\/collections\/activites\/[0-9]{4}/)
			) {
				return false;
			}
		...
		photos: async (data) => {
			...
			const cacheDir = path.join(
				"src/_cache/photos/",
				path.dirname(data.page.filePathStem),
			);
			...
			if (fs.existsSync(photosPath) && fs.lstatSync(photosPath).isDirectory()) {
				const photosDataPromises = fs
					...
					.map(async (file) => {
						...
						let photoExif;
						try {
							photoExif = await exifr.parse(
								path.join(photosPath, file),
								EXIFR_OPTIONS,
							);
						} catch (err) {
							console.error(
						...
						if (!photoExif) {
							console.warn(
								`No EXIF data found for photo: ${path.join(photosPath, photo.src)}`,
							);
						}
						...
						if (
							...
							luxonDate = DateTime.fromHTTP(
								...
								`UTC+${Number.parseInt(photoExif.exif.OffsetTime, 10)}`,
							);
						...
						if (
							...
							console.warn(
								`No dimensions in EXIF for ${path.join(photosPath, photo.src)}`,
							);
						}
		...
		trace: (data) => {
			...
			if (fs.existsSync(gpsbabelFile)) {
				...
				const originalGpxFile = path.join(
					"src",
					path.dirname(data.page.filePathStem),
					"sources/original.gpx",
				);
				if (fs.existsSync(originalGpxFile)) {
					...
					execSync(
						`/opt/homebrew/bin/gpsbabel -r -i gpx -f ${originalGpxFile} -x simplify,error=0.0005k -o gpx -F ${gpsbabelFile}`,
					);
					gpxContentFile = gpsbabelFile;
				}
			}
			...
			if (gpxContentFile) {
				const gpxContent = new DOMParser().parseFromString(
					fs.readFileSync(gpxContentFile, "utf8"),
				);
				...
				const siteDir = path.join(
					"_site",
					path.dirname(data.page.filePathStem).replace(/^\/collections\//, ""),
				);
				if (!fs.existsSync(siteDir)) {
					fs.mkdirSync(siteDir, { recursive: true });
				}
				fs.writeFileSync(
					path.join(siteDir, "trace.geojson"),
					geoJSONString,
					"utf8",
				);
