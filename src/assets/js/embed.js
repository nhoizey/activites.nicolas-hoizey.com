...
(async (window) => {
	...
	const highligthRoute = (map, activityId, bbox) => {
		map.setPaintProperty(
			...
			"line-width",
			route_settings.route_highlight_width +
	...
	const unhighligthRoute = (map, activityId, bbox) => {
		map.setPaintProperty(
			`route-${activityId}-shadow`,
			"line-width",
			route_settings.route_width + route_settings.route_shadow_additional_width,
		);
		map
			.setPaintProperty(
				`route-${activityId}`,
			...
			for (let [activityId, geoJsonData] of Object.entries(embedData)) {
				if (geoJsonData === false) {
					geoJsonData = await fetchGeojson(
						`/activites/${activityId}trace.geojson`,
					);
				...
				map.on("load", () => {
					...
					for (const [activityId, geoJsonData] of Object.entries(embedData)) {
						const activityBbox = bbox(
							lineString(geoJsonData.features[0].geometry.coordinates),
						);
						...
						const activity = embed.querySelector(
