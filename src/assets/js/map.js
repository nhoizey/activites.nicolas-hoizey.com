...
import { colorsByType, colorsOnDark } from "../../_data/colors.js";
...
(async (window) => {
	...
	// France bounding box
	const franceBbox = [-5.1406, 41.3337, 9.6625, 51.089];
	...
	const selectRoute = (event, map, activityId, bbox) => {
		...
		map
			.setPaintProperty(`route-${activityId}-shadow`, "line-width", [
				"interpolate",
				["linear"],
				["zoom"],
				0,
				...
				route_settings.route_width +
					route_settings.route_shadow_additional_width * 2,
			])
			.setPaintProperty(
				`route-${activityId}-shadow`,
				"line-opacity",
				route_settings.route_shadow_opacity,
			);
		map.setPaintProperty(
			`route-${activityId}`,
			"line-opacity",
			route_settings.route_highlight_opacity,
		);
...
<svg class="icon" aria-hidden="true" viewBox="0 0 24 24"><use xlink:href="#date-icon" /></svg> ${new Date(
			geoJsonDatas[activityId].date,
...
<p><a href="${geoJsonDatas[activityId].url}">Voir la page de l'activité</a></p>`;
	...
	const unSelectRoute = (map, activityId) => {
		...
		map
			.setPaintProperty(`route-${activityId}`, "line-width", [
				"interpolate",
				["linear"],
				["zoom"],
				0,
				...
				onAdd(map) {
					...
					const updateActivities = () => {
						...
						for (const [traceId, geoJsonData] of Object.entries(geoJsonDatas)) {
							...
							if (
								currentFilters.includes(geoJsonData.type) &&
								currentMonths.includes(geoJsonData.month)
							) {
								// show
								if (typeof geoJsonData.trace !== "string") {
									// TODO: load and render trace if not done yet
									map.setLayoutProperty(
										`route-${traceId}-shadow`,
										"visibility",
										"visible",
									);
									map.setLayoutProperty(
...
`;
					...
					const updateSlidersLabels = () => {
						...
						const newFromMonth = Number.parseInt(
							div.querySelector("#fromSlider").value,
							10,
						);
						div.querySelector("#fromMonth").textContent =
							readableMonths[newFromMonth];

						const newToMonth = Number.parseInt(
							div.querySelector("#toSlider").value,
							10,
						);
						div.querySelector("#toMonth").textContent =
							readableMonths[newToMonth];
...
`;
			...
			for (const [traceId, geoJsonData] of Object.entries(geoJsonDatas)) {
				...
				const activityBbox = bbox(
					lineString(geoJsonData.trace.features[0].geometry.coordinates),
				);
				...
				map.addLayer({
					...
					paint: {
						"line-color":
							colorsByType[geoJsonData.trace.features[0].properties.type] ||
							colorsOnDark[traceIndex % colorsOnDark.length],
						"line-width": [
							"interpolate",
							["linear"],
							["zoom"],
